...
const LOCALES = {
  ...
  ar: {
    ...
    provider_mismatch_warning: (m,p)=>`قد لا يعمل "${m}" مع مزودك المُعد (${p}). أرسل على أي حال، أو شغّل \`hermes model\` في الطرفية للتبديل.`,
    ...
    no_personalities: 'لم يتم العثور على شخصيات (أضفها إلى ~/.hermes/personalities/)',
    ...
    settings_desc_sync_insights: 'يعكس استخدام الرموز من WebUI إلى state.db حتى يتضمن hermes /insights بيانات جلسات المتصفح. معطّل افتراضياً.',
    ...
    providers_oauth_config_yaml_hint: 'تم إعداد الرمز عبر config.yaml. للتحديث، عدّل قسم المزودين في config.yaml أو شغّل hermes auth.',
    providers_oauth_not_configured_hint: 'لم تتم المصادقة. شغّل hermes auth في الطرفية لإعداد هذا المزود.',
    providers_save: 'حفظ',
    ...
    onboarding_oauth_provider_not_ready_body: 'هذه المثيل مُعد لاستخدام <strong>{provider}</strong>، والذي يستخدم OAuth بدلاً من مفتاح API. شغّل <code>hermes auth</code> أو <code>hermes model</code> في طرفية للمصادقة، ثم أعد تحميل Web UI.',
