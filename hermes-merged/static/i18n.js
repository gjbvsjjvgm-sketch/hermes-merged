...
const LOCALES = {
  en: {
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" may not work with your configured provider (${p}). Send anyway, or run \`ym model\` in your terminal to switch.`,
  ...
  untitled:'Untitled',
    no_personalities: 'No personalities found (add them to ~/.yusuf-mussa/personalities/)',
  ...
  folder_add_as_space_title: 'Add as Space?',
    ...
    settings_section_preferences_meta: 'Defaults and UI behavior for Yusuf Mussa Web UI.',
    ...
    settings_desc_cli_sessions: 'Merges sessions from the Yusuf Mussa CLI (state.db) into the session list. Click a CLI session to import it and continue the conversation.',
    settings_desc_sync_insights: 'Mirrors WebUI token usage to state.db so ym /insights includes browser session data. Off by default.',
    ...
    settings_desc_bot_name: 'Display name for the assistant throughout the UI. Defaults to Yusuf Mussa.',
    ...
    providers_oauth_config_yaml_hint: 'Token configured via config.yaml. To update, edit the providers section in your config.yaml or run ym auth.',
    providers_oauth_not_configured_hint: 'Not authenticated. Run ym auth in the terminal to configure this provider.',
    providers_save: 'Save',
    ...
    suggest_search: 'Search the web for latest news',
    suggest_code: 'Write and run a Python script',
    suggest_arabic: 'Help me in Arabic',
    // onboarding
    onboarding_badge: 'FIRST RUN',
    onboarding_title: 'Welcome to Yusuf Mussa Web UI',
    onboarding_lead: 'A quick guided setup will verify Yusuf Mussa, save a real provider configuration, choose a workspace and model, and optionally protect the app with a password.',
    onboarding_back: 'Back',
    onboarding_continue: 'Continue',
    onboarding_skip: 'Skip setup',
    onboarding_skipped: 'Setup skipped — using existing config.',
    onboarding_open: 'Open Yusuf Mussa',
    onboarding_step_system_title: 'System check',
    onboarding_step_system_desc: 'Verify Yusuf Mussa Agent and config visibility.',
    onboarding_step_setup_title: 'Provider setup',
    onboarding_step_setup_desc: 'Save the minimum Yusuf Mussa provider config.',
    ...
    onboarding_notice_system_ready: 'Yusuf Mussa Agent looks reachable from the Web UI.',
    onboarding_notice_system_unavailable: 'Yusuf Mussa Agent is not fully available yet. Bootstrap can install it, but provider setup may still require a terminal.',
    onboarding_check_agent: 'Yusuf Mussa Agent',
    ...
    onboarding_notice_setup_required: 'Choose a simple provider path here. Advanced OAuth flows still belong in the Yusuf Mussa CLI for now.',
    onboarding_notice_setup_already_ready: 'A working Yusuf Mussa provider setup is already detected. You can keep it or replace it here.',
    onboarding_oauth_provider_ready_title: 'Provider already authenticated',
    onboarding_oauth_provider_ready_body: 'This instance is configured to use an OAuth provider (<strong>{provider}</strong>) that was set up via the Yusuf Mussa CLI. No API key is needed here — click Continue to finish setup.',
    onboarding_oauth_provider_not_ready_title: 'OAuth provider not yet authenticated',
    onboarding_oauth_provider_not_ready_body: 'This instance is configured to use <strong>{provider}</strong>, which uses OAuth rather than an API key. Run <code>ym auth</code> or <code>ym model</code> in a terminal to authenticate, then reload the Web UI.',
    ...
    onboarding_api_key_help_prefix: 'Saved as a secret in your Yusuf Mussa .env file using',
    ...
    onboarding_workspace_help: 'Pick the model Yusuf Mussa should use for new chats after setup completes.',
  ...
  ru: {
    ...
    `"${m}" может не работать с вашим настроенным провайдером (${p}). Всё равно отправить или запустите \`ym model\` в терминале, чтобы переключиться.`,
    ...
    cmd_skills: 'Показать доступные навыки Yusuf Mussa',
    ...
    no_personalities: 'Личности не найдены (добавьте их в ~/.yusuf-mussa/personalities/)',
  ...
  folder_add_as_space_title: 'Добавить как пространство?',
    ...
    settings_desc_cli_sessions: 'Объединяет сеансы из Yusuf Mussa CLI (state.db) в список сеансов. Нажмите на CLI-сеанс, чтобы импортировать его и продолжить разговор.',
    settings_desc_sync_insights: 'Синхронизирует использование токенов WebUI в state.db, чтобы Yusuf Mussa /insights включал данные браузерных сеансов. Выключено по умолчанию.',
    ...
    settings_desc_bot_name: 'Отображаемое имя помощника во всём интерфейсе. По умолчанию Yusuf Mussa.',
    ...
    onboarding_lead: 'Краткая пошаговая настройка проверит Yusuf Mussa, сохранит рабочую конфигурацию провайдера, выберет рабочее пространство и модель и при желании защитит приложение паролем.',
    ...
    onboarding_open: 'Открыть Yusuf Mussa',
    onboarding_step_system_title: 'Проверка системы',
    onboarding_step_system_desc: 'Проверить Yusuf Mussa Agent и видимость конфигурации.',
    onboarding_step_setup_title: 'Настройка провайдера',
    onboarding_step_setup_desc: 'Сохранить минимальную рабочую конфигурацию провайдера Yusuf Mussa.',
    ...
    onboarding_notice_system_ready: 'Yusuf Mussa Agent, похоже, доступен из Web UI.',
    onboarding_notice_system_unavailable: 'Yusuf Mussa Agent ещё не полностью доступен. Bootstrap может установить его, но для настройки провайдера всё ещё может понадобиться терминал.',
    onboarding_check_agent: 'Yusuf Mussa Agent',
    ...
    onboarding_notice_setup_required: 'Выберите здесь простой путь настройки провайдера. Продвинутые OAuth-сценарии пока остаются в Yusuf Mussa CLI.',
    onboarding_notice_setup_already_ready: 'Уже обнаружена рабочая конфигурация провайдера Yusuf Mussa. Вы можете оставить её или заменить здесь.',
    onboarding_oauth_provider_ready_title: 'Провайдер уже авторизован',
    onboarding_oauth_provider_ready_body: 'Этот экземпляр настроен на использование OAuth-провайдера (<strong>{provider}</strong>), настроенного через Yusuf Mussa CLI. API-ключ здесь не нужен — нажмите «Продолжить», чтобы завершить настройку.',
    onboarding_oauth_provider_not_ready_title: 'OAuth-провайдер ещё не авторизован',
    onboarding_oauth_provider_not_ready_body: 'Этот экземпляр настроен на использование <strong>{provider}</strong>, который работает через OAuth, а не через API-ключ. Запустите <code>ym auth</code> или <code>ym model</code> в терминале, чтобы пройти авторизацию, затем обновите Web UI.',
    ...
    onboarding_api_key_help_prefix: 'Сохраняется как секрет в вашем файле `.env` Yusuf Mussa с помощью',
    ...
    onboarding_workspace_help: 'Выберите модель, которую Yusuf Mussa должен использовать для новых чатов после завершения настройки.',
  ...
  es: {
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" puede no funcionar con tu proveedor configurado (${p}). Envía de todas formas, o ejecuta \`ym model\` en la terminal para cambiar.`,
    ...
    cmd_skills: 'Listar las skills de Yusuf Mussa disponibles',
    ...
    no_personalities: 'No se encontraron personalidades (añádelas a ~/.yusuf-mussa/personalities/)',
  ...
  folder_add_as_space_title: '¿Añadir como espacio?',
    ...
    settings_desc_cli_sessions: 'Fusiona las sesiones del CLI de Yusuf Mussa (state.db) en la lista de sesiones. Haz clic en una sesión de CLI para importarla y continuar la conversación.',
    settings_desc_sync_insights: 'Refleja el uso de tokens de la WebUI en state.db para que ym /insights incluya datos de sesiones del navegador. Desactivado por defecto.',
    ...
    settings_desc_bot_name: 'Nombre visible del asistente en toda la UI. Por defecto es Yusuf Mussa.',
    ...
    onboarding_title: 'Bienvenido a Yusuf Mussa Web UI',
    onboarding_lead: 'Una guía rápida verificará Yusuf Mussa, guardará una configuración real del proveedor, elegirá un espacio de trabajo y un modelo, y opcionalmente protegerá la app con una contraseña.',
    ...
    onboarding_open: 'Abrir Yusuf Mussa',
    onboarding_step_system_title: 'Comprobación del sistema',
    onboarding_step_system_desc: 'Verifica Yusuf Mussa Agent y la visibilidad de la configuración.',
    onboarding_step_setup_title: 'Configuración del proveedor',
    onboarding_step_setup_desc: 'Guarda la configuración mínima real de Yusuf Mussa.',
    ...
    onboarding_notice_system_ready: 'Parece que Yusuf Mussa Agent está accesible desde la Web UI.',
    onboarding_notice_system_unavailable: 'Yusuf Mussa Agent todavía no está totalmente disponible. Bootstrap puede instalarlo, pero la configuración del proveedor quizá aún requiera una terminal.',
    onboarding_check_agent: 'Yusuf Mussa Agent',
    ...
    onboarding_notice_setup_required: 'Elige aquí una ruta simple de proveedor. Los flujos OAuth avanzados siguen siendo del CLI de Yusuf Mussa por ahora.',
    onboarding_notice_setup_already_ready: 'Ya se detectó una configuración funcional del proveedor de Yusuf Mussa. Puedes conservarla o reemplazarla aquí.',
    onboarding_oauth_provider_ready_title: 'Proveedor ya autenticado',
    onboarding_oauth_provider_ready_body: 'Esta instancia está configurada para usar un proveedor OAuth (<strong>{provider}</strong>) configurado mediante la CLI de Yusuf Mussa. No se necesita clave API aquí — haz clic en Continuar para finalizar la configuración.',
    onboarding_oauth_provider_not_ready_title: 'Proveedor OAuth no autenticado aún',
    onboarding_oauth_provider_not_ready_body: 'Esta instancia está configurada para usar <strong>{provider}</strong>, que utiliza OAuth en lugar de una clave API. Ejecuta <code>ym auth</code> o <code>ym model</code> en una terminal para autenticarte y recarga la interfaz web.',
    ...
    onboarding_api_key_help_prefix: 'Se guarda como secreto en tu archivo .env de Yusuf Mussa usando',
    onboarding_base_url_label: 'Base URL',
    ...
    onboarding_workspace_help: 'Elige el modelo que Yusuf Mussa debe usar para nuevos chats cuando termine la configuración.',
  ...
  de: {
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" funktioniert möglicherweise nicht mit Ihrem konfigurierten Provider (${p}). Trotzdem senden, oder \`ym model\` im Terminal ausführen.`,
    ...
    cmd_skills: 'Verfügbare Yusuf Mussa-Skills auflisten',
    ...
    no_personalities: 'Keine Persönlichkeiten gefunden (füge sie in ~/.yusuf-mussa/personalities/ hinzu)',
  ...
  folder_add_as_space_title: 'Als Bereich hinzufügen?',
    ...
    settings_desc_cli_sessions: 'Fügt Sitzungen aus der Yusuf Mussa CLI (state.db) in die Sitzungsliste ein. Klicken Sie auf eine CLI-Sitzung, um sie zu importieren und das Gespräch fortzusetzen.',
    settings_desc_sync_insights: 'Spiegelt den WebUI-Token-Verbrauch in die state.db, sodass ym /insights Browser-Sitzungsdaten enthält. Standardmäßig aus.',
    ...
    settings_desc_bot_name: 'Anzeigename für den Assistenten in der UI. Standardmäßig Yusuf Mussa.',
    ...
    onboarding_title: 'Willkommen bei Yusuf Mussa',
    ...
    onboarding_step_system_desc: 'Yusuf Mussa Agent muss installiert und erreichbar sein.',
    ...
    onboarding_notice_system_unavailable: 'Agent nicht erreichbar. Bitte starten Sie Yusuf Mussa Agent.',
  ...
  zh: {
    ...
    provider_mismatch_warning: (m,p)=>`\"${m}\" \u53ef\u80fd\u65e0\u6cd5\u5728\u5f53\u524d\u914d\u7f6e\u7684\u63d0\u4f9b\u5546 (${p}) \u4e0b\u5de5\u4f5c\u3002\u76f4\u63a5\u53d1\u9001\uff0c\u6216\u5728\u7ec8\u7aef\u8fd0\u884c \`ym model\` \u5207\u6362\u3002`,
    ...
    cmd_skills: '\u5217\u51fa\u53ef\u7528\u7684 Yusuf Mussa \u6280\u80fd',
    ...
    no_personalities: '\u6ca1\u6709\u627e\u5230\u4eba\u8bbe\uff08\u53ef\u6dfb\u52a0\u5230 ~/.yusuf-mussa/personalities/\uff09',
  ...
  folder_add_as_space_title: '\u6dfb\u52a0\u4e3a\u5de5\u4f5c\u533a\uff1f',
    ...
    settings_desc_cli_sessions: '将 Yusuf Mussa CLI（state.db）中的会话合并到会话列表。点击某个 CLI 会话可导入并继续对话。',
    settings_desc_sync_insights: '将 WebUI token 使用情况同步到 state.db，使 ym /insights 包含浏览器会话数据。默认关闭。',
    settings_desc_check_updates: '当有更新的 WebUI 或助手版本时显示横幅。会在后台定期执行 git fetch。',
    settings_desc_bot_name: '助手在 UI 中的显示名称。默认为 Yusuf Mussa。',
    ...
    onboarding_lead: '快速引导将验证 Yusuf Mussa、保存真实的提供商配置、选择工作区和模型，并可选设置密码保护应用。',
    onboarding_back: '返回',
    onboarding_continue: '继续',
    onboarding_skip: '跳过设置',
    onboarding_skipped: '设置已跳过 — 使用现有配置。',
    onboarding_open: '打开 Yusuf Mussa',
    onboarding_step_system_title: '系统检查',
    onboarding_step_system_desc: '验证 Yusuf Mussa Agent 与配置可见性。',
    onboarding_step_setup_title: '提供商设置',
    onboarding_step_setup_desc: '保存最小可用的 Yusuf Mussa 提供商配置。',
    ...
    onboarding_notice_system_ready: 'Yusuf Mussa Agent 看起来可从 Web UI 访问。',
    onboarding_notice_system_unavailable: 'Yusuf Mussa Agent 尚未完全可用。Bootstrap 可以安装它，但提供商设置可能仍需要终端。',
    onboarding_check_agent: 'Yusuf Mussa Agent',
    ...
    onboarding_notice_setup_required: '请先在此选择一个简单的提供商路径。高级 OAuth 流程暂时仍建议在 Yusuf Mussa CLI 中完成。',
    onboarding_notice_setup_already_ready: '已检测到可用的 Yusuf Mussa 提供商配置。你可以保留它，或在这里替换。',
    onboarding_oauth_provider_ready_title: '提供商已完成认证',
    onboarding_oauth_provider_ready_body: '此实例已配置为使用通过 Yusuf Mussa CLI 设置的 OAuth 提供商（<strong>{provider}</strong>）。这里不需要 API key，点击继续即可完成设置。',
    onboarding_oauth_provider_not_ready_title: 'OAuth 提供商尚未认证',
    onboarding_oauth_provider_not_ready_body: '此实例已配置为使用 <strong>{provider}</strong>，该提供商使用 OAuth 而非 API key。请在终端运行 <code>ym auth</code> 或 <code>ym model</code> 完成认证，然后重新加载 Web UI。',
    ...
    onboarding_api_key_help_prefix: '会作为密钥保存到 Yusuf Mussa .env 文件中，变量名为',
    onboarding_base_url_label: 'Base URL',
    onboarding_base_url_placeholder: 'https://your-endpoint.example/v1',
    onboarding_base_url_help: '用于 OpenAI 兼容路由、自托管服务、LiteLLM、Ollama、LM Studio、vLLM 或类似端点。',
    onboarding_model_label: '默认模型',
    onboarding_workspace_help: '选择设置完成后 Yusuf Mussa 在新聊天中使用的模型。',
    onboarding_custom_model_placeholder: 'your-model-name',
  ...
  'zh-Hant': {
    ...
    provider_mismatch_warning: (m,p)=>`\"${m}\" \u53ef\u80fd\u7121\u6cd5\u5728\u7576\u524d\u914d\u7f6e\u7684\u63d0\u4f9b\u8005 (${p}) \u4e0b\u904b\u4f5c\u3002\u5c1a\u9001\uff0c\u6216\u5728\u7d42\u7aef\u57f7\u884c \`ym model\` \u5207\u63db\u3002`,
    ...
    no_personalities: '\u6c92\u6709\u627e\u5230\u4eba\u8a2d\uff08\u53ef\u6dfb\u52a0\u5230 ~/.yusuf-mussa/personalities/\uff09',
  ...
  folder_add_as_space_title: '\u65b0\u589e\u70ba\u5de5\u4f5c\u5340\uff1f',
    ...
    settings_section_preferences_meta: 'Yusuf Mussa Web UI 的預設值與介面行為。',
    ...
    settings_desc_cli_sessions: '將 Yusuf Mussa CLI (的 state.db) 中的會話添加到會話清單。點擊一個 CLI 會話將導入它並繼續對話。',
    settings_desc_sync_insights: '將 WebUI token 使用情況同步到 state.db，使 ym /insights 包含瀏覽器會話數據。預設未啟用。',
    ...
    n_messages: (n) => `${n} 則訊息`,
    load_older_messages: '↑ 向上捲動或點擊以載入較早的訊息',
    onboarding_api_key_help_prefix: '\u900f\u904e\u4ee5\u4e0b\u65b9\u5f0f\u5132\u5b58\u70ba Yusuf Mussa .env \u6a94\u6848\u4e2d\u7684\u6a5f\u5bc6',
    ...
    onboarding_lead: '\u5feb\u901f\u9010\u6b65\u8a2d\u5b9a\uff1a\u6aa2\u67e5 Yusuf Mussa\u3001\u5132\u5b58\u63d0\u4f9b\u8005\u914d\u7f6e\u3001\u9078\u64c7\u5de5\u4f5c\u5340\u8207\u6a21\u578b\uff0c\u4e26\u53ef\u9078\u64c7\u52a0\u5165\u5bc6\u78bc\u4fdd\u8b77\u3002',
    ...
    onboarding_notice_setup_already_ready: '\u5df2\u5075\u6e2c\u5230\u53ef\u7528\u7684 Yusuf Mussa \u63d0\u4f9b\u8005\u914d\u7f6e\u3002\u60a8\u53ef\u4ee5\u4fdd\u7559\u6216\u5728\u6b64\u53d6\u4ee3\u3002',
    onboarding_notice_setup_required: '\u5728\u6b64\u9078\u64c7\u5feb\u901f\u63d0\u4f9b\u8005\u8a2d\u5b9a\u8def\u5f91\u3002\u9032\u968e OAuth \u8a2d\u5b9a\u4ecd\u8acb\u4f7f\u7528 Yusuf Mussa CLI\u3002',
    onboarding_notice_system_ready: 'Yusuf Mussa Agent \u53ef\u5f9e Web UI \u5b58\u53d6\u3002',
    onboarding_notice_system_unavailable: 'Yusuf Mussa Agent \u5c1a\u672a\u5b8c\u5168\u53ef\u7528\u3002\u5f15\u5c0e\u7a0b\u5f0f\u53ef\u80fd\u5b89\u88dd\u5b83\uff0c\u4f46\u63d0\u4f9b\u8005\u8a2d\u5b9a\u53ef\u80fd\u4ecd\u9700\u7d42\u7aef\u6a5f\u3002',
    onboarding_notice_workspace: '\u9019\u4e9b\u503c\u4f7f\u7528\u8207\u4e00\u822c\u61c9\u7528\u7a0b\u5f0f\u76f8\u540c\u7684\u8a2d\u5b9a API\u3002',
    onboarding_oauth_provider_not_ready_body: '\u6b64\u5be6\u4f8b\u8a2d\u5b9a\u4f7f\u7528 <strong>{provider}</strong>\uff0c\u5b83\u4f7f\u7528 OAuth \u800c\u975e API \u91d1\u9470\u3002\u8acb\u5728\u7d42\u7aef\u6a5f\u4e2d\u57f7\u884c <code>ym auth</code> \u6216 <code>ym model</code> \u9032\u884c\u6388\u6b0a\uff0c\u7136\u5f8c\u91cd\u65b0\u6574\u7406 Web UI\u3002',
    onboarding_oauth_provider_not_ready_title: 'OAuth \u63d0\u4f9b\u8005\u5c1a\u672a\u6388\u6b0a',
    onboarding_oauth_provider_ready_body: '\u6b64\u5be6\u4f8b\u8a2d\u5b9a\u4f7f\u7528\u900f\u904e Yusuf Mussa CLI \u914d\u7f6e\u7684 OAuth \u63d0\u4f9b\u8005\uff08<strong>{provider}</strong>\uff09\u3002\u7121\u9700 API \u91d1\u9470 \u2014 \u6309\u7e7c\u7e8c\u5b8c\u6210\u8a2d\u5b9a\u3002',
    onboarding_oauth_provider_ready_title: '\u63d0\u4f9b\u8005\u5df2\u6388\u6b0a',
    onboarding_oauth_switch_hint: '\u6216\u9078\u64c7\u4e0b\u65b9\u5176\u4ed6\u63d0\u4f9b\u8005\u4ee5\u5207\u63db\u5230 API \u91d1\u9470\u8a2d\u5b9a\uff1a',
    onboarding_open: '\u958b\u555f Yusuf Mussa',
    ...
    onboarding_step_setup_desc: '\u5132\u5b58\u6700\u5c0f\u53ef\u7528\u7684 Yusuf Mussa \u63d0\u4f9b\u8005\u914d\u7f6e\u3002',
    onboarding_step_setup_title: '\u63d0\u4f9b\u8005\u8a2d\u5b9a',
    onboarding_step_system_desc: '\u9a57\u8b49 Yusuf Mussa Agent \u8207\u914d\u7f6e\u53ef\u898b\u6027\u3002',
    onboarding_step_system_title: '\u7cfb\u7d71\u6aa2\u67e5',
    onboarding_step_workspace_desc: '\u70ba\u65b0\u6703\u8a71\u9078\u64c7\u9810\u8a2d\u503c\u3002',
    onboarding_step_workspace_title: '\u5de5\u4f5c\u5340\u8207\u6a21\u578b',
    onboarding_title: '\u6b61\u8fce\u4f7f\u7528 Yusuf Mussa Web UI',
    onboarding_unknown: '\u672a\u77e5',
    onboarding_workspace_help: '\u9078\u64c7 Yusuf Mussa \u5728\u8a2d\u5b9a\u5b8c\u6210\u5f8c\u7528\u65bc\u65b0\u804a\u5929\u5ba4\u7684\u6a21\u578b\u3002',
    ...
    providers_oauth_config_yaml_hint: '\u5df2\u900f\u904e config.yaml \u8a2d\u5b9a\u6b0a\u8b49\u3002\u8981\u66f4\u65b0\uff0c\u8acb\u7de8\u8f2f config.yaml \u4e2d\u7684 providers \u5340\u6bb5\uff0c\u6216\u57f7\u884c ym auth\u3002',
    providers_oauth_not_configured_hint: '\u5c1a\u672a\u9a57\u8b49\u3002\u8acb\u5728\u7d42\u7aef\u6a5f\u57f7\u884c ym auth \u4ee5\u8a2d\u5b9a\u6b64\u63d0\u4f9b\u8005\u3002',
    providers_remove: '\u79fb\u9664',
  ...
  pt: {
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" pode não funcionar com seu provedor configurado (${p}). Enviar assim mesmo, ou execute \`ym model\` no terminal para trocar.`,
    ...
    cmd_skills: 'Listar skills disponíveis do Yusuf Mussa',
    ...
    no_personalities: 'Nenhuma personalidade encontrada (adicione em ~/.yusuf-mussa/personalities/)',
    ...
    settings_section_preferences_meta: 'Padrões e comportamento UI do Yusuf Mussa Web UI.',
    ...
    settings_desc_cli_sessions: 'Mescla sessões do Yusuf Mussa CLI na lista. Clique para importar.',
    ...
    settings_desc_bot_name: 'Nome de exibição do assistente. Padrão: Yusuf Mussa.',
    ...
    providers_oauth_config_yaml_hint: 'Token configurado via config.yaml. Para atualizar, edite config.yaml ou rode ym auth.',
    providers_oauth_not_configured_hint: 'Não autenticado. Rode ym auth no terminal.',
    ...
    onboarding_title: 'Bem-vindo ao Yusuf Mussa Web UI',
    onboarding_lead: 'Uma configuração rápida vai verificar Yusuf Mussa, salvar provedor, escolher workspace e modelo, e opcionalmente proteger com senha.',
    ...
    onboarding_step_system_desc: 'Verificar Yusuf Mussa Agent e visibilidade da config.',
    onboarding_step_setup_title: 'Configuração do provedor',
    onboarding_step_setup_desc: 'Salvar config mínima do provedor Yusuf Mussa.',
    ...
    onboarding_notice_system_ready: 'Yusuf Mussa Agent parece acessível pela Web UI.',
    onboarding_notice_system_unavailable: 'Yusuf Mussa Agent não está totalmente disponível. Bootstrap pode instalar, mas setup do provedor pode requerer terminal.',
    onboarding_check_agent: 'Yusuf Mussa Agent',
    ...
    onboarding_notice_setup_required: 'Escolha caminho simples de provedor aqui. OAuth avançado ainda pertence ao Yusuf Mussa CLI.',
    onboarding_notice_setup_already_ready: 'Setup de provedor Yusuf Mussa já detectado. Pode manter ou substituir.',
    ...
    onboarding_oauth_provider_not_ready_body: 'Esta instância usa <strong>{provider}</strong> com OAuth. Rode `ym auth` no terminal.',
    ...
    onboarding_api_key_help_prefix: 'Salvo como segredo no .env do Yusuf Mussa usando',
    onboarding_base_url_label: 'Base URL',
    ...
    onboarding_workspace_help: 'Escolha modelo que Yusuf Mussa deve usar para novos chats.',
  ...
  ko: {
    ...
    settings_section_preferences_meta: 'Yusuf Mussa Web UI의 기본값과 UI 동작.',
    ...
    settings_desc_cli_sessions: 'Yusuf Mussa CLI(state.db)의 세션을 세션 목록에 병합합니다. CLI 세션을 클릭하면 가져와서 대화를 계속할 수 있습니다.',
    settings_desc_sync_insights: 'WebUI 토큰 사용량을 state.db에 반영하여 ym /insights에 브라우저 세션 데이터가 포함되도록 합니다. 기본값은 꺼짐입니다.',
    settings_desc_check_updates: 'WebUI 또는 Agent의 새 버전이 있으면 배너를 표시합니다. 백그라운드에서 주기적으로 git fetch를 실행합니다.',
    settings_desc_bot_name: 'UI 전체에 표시되는 Assistant 이름입니다. 기본값은 Yusuf Mussa입니다.',
    ...
    onboarding_lead: '간단한 안내 설정으로 Yusuf Mussa 설치를 확인하고, 워크스페이스와 모델을 선택하며, 필요하면 비밀번호로 앱을 보호합니다.',
    onboarding_back: '뒤로',
  ...
  ar: {
    ...
    mic_no_speech: 'لم يتم اكتشاف كلام. حاول مرة أخرى.',
    mic_network: 'التعرف على الكلام غير متوفر.',
    mic_error: 'خطأ في الإدخال الصوتي: ',
    ...
    diff_loading: 'جاري تحميل الفرق',
    diff_error: 'تعذر تحميل ملف التصحيح',
    diff_too_large: 'ملف التصحيح كبير جداً للعرض المباشر',
    tree_view: 'شجري',
    raw_view: 'خام',
    parse_failed_note: 'فشل التحليل',
    you: 'أنت',
    mcp_servers_title: 'خوادم MCP',
    mcp_servers_desc: 'إدارة خوادم MCP المُعدَّة في config.yaml.',
    mcp_no_servers: 'لا توجد خوادم MCP مُعدَّة.',
    mcp_add_server: '+ إضافة خادم',
    mcp_field_name: 'اسم الخادم',
    mcp_transport_label: 'نوع النقل',
    mcp_field_command: 'الأمر',
    mcp_field_args: 'الوسائط (مفصولة بفواصل)',
    mcp_field_url: 'الرابط',
    mcp_field_timeout: 'المهلة (بالثواني)',
    mcp_save: 'حفظ',
    mcp_cancel: 'إلغاء',
    mcp_name_required: 'اسم الخادم مطلوب.',
    mcp_url_required: 'الرابط مطلوب لنقل HTTP.',
    mcp_command_required: 'الأمر مطلوب لنقل stdio.',
    mcp_saved: 'تم حفظ خادم MCP.',
    mcp_save_failed: 'فشل حفظ خادم MCP.',
    mcp_delete_confirm_title: 'حذف خادم MCP',
    mcp_delete_confirm_message: 'حذف خادم MCP "{0}"؟ لا يمكن التراجع عن هذا الإجراء.',
    mcp_deleted: 'تم حذف خادم MCP.',
    mcp_delete_failed: 'فشل حذف خادم MCP.',
    mcp_load_failed: 'فشل تحميل خوادم MCP.',
    // PDF preview (#480)
    pdf_loading: 'جاري تحميل PDF {0}\u2026',
    pdf_too_large: 'ملف PDF كبير جداً للمعاينة المباشرة',
    pdf_no_pages: 'ملف PDF لا يحتوي على صفحات',
    pdf_error: 'فشل عرض معاينة PDF',
    pdf_download: 'تنزيل PDF',
    // HTML sandbox preview (#482)
    html_loading: 'جاري تحميل معاينة HTML\u2026',
    html_too_large: 'ملف HTML كبير جداً للمعاينة المباشرة',
    html_error: 'فشل عرض معاينة HTML',
    html_open_full: 'فتح الصفحة الكاملة',
    html_sandbox_label: 'معاينة HTML (بيئة معزولة)',
    thinking: 'يفكر',
    expand_all: 'توسيع الكل',
    collapse_all: 'طي الكل',
    edit_failed: 'فشل التعديل: ',
    regen_failed: 'فشل إعادة الإنشاء: ',
    reconnect_active: 'لا يزال يتم إنشاء رد. إعادة التحميل عند الاستعداد؟',
    reconnect_finished: 'كان رد قيد التقدم عندما غادرت آخر مرة. قد تكون الرسائل قد تحددت.',
    // approval card
    approval_heading: 'يتطلب موافقة',
    approval_desc_prefix: 'تم اكتشاف أمر خطير',
    approval_btn_once: 'السماح مرة واحدة',
    approval_btn_once_title: 'السماح بهذا الأمر فقط (Enter)',
    approval_btn_session: 'السماح للجلسة',
    approval_btn_session_title: 'السماح طوال جلسة المحادثة هذه',
    approval_btn_always: 'السماح دائماً',
    approval_btn_always_title: 'السماح دائماً بنمط الأمر هذا',
    approval_btn_deny: 'رفض',
    approval_btn_deny_title: 'رفض — عدم تشغيل هذا الأمر',
    approval_responding: 'جاري الرد\u2026',
    clarify_heading: 'يلزم توضيح',
    clarify_hint: 'اختر خياراً، أو اكتب إجابتك أدناه.',
    clarify_other: 'أخرى',
    clarify_send: 'إرسال',
    clarify_input_placeholder: 'اكتب ردك\u2026',
    clarify_responding: 'جاري الرد\u2026',
    untitled: 'بدون عنوان',
    n_messages: (n) => `${n} رسائل`,
    load_older_messages: '↑ مرر لأعلى أو انقر لتحميل الرسائل الأقدم',
    queued_label: 'يُرسل بعد الرد',
    queued_count: (n) => n === 1 ? '1 في الانتظار' : `${n} في الانتظار`,
    queued_cancel: 'إلغاء الرسالة في الانتظار',
    model_unavailable: ' (غير متوفر)',
    model_unavailable_title: 'هذا النموذج لم يعد في قائمة المزودين الحاليين',
    provider_mismatch_warning: (m,p)=>`قد لا يعمل "${m}" مع مزودك المُعد (${p}). أرسل على أي حال، أو شغّل \`ym model\` في الطرفية للتبديل.`,
    provider_mismatch_label: 'عدم تطابق المزود',
    model_not_found_label: 'النموذج غير موجود',
    model_custom_label: 'معرّف نموذج مخصص',
    model_custom_placeholder: 'مثال openai/gpt-5.4',
    model_search_placeholder: 'البحث في النماذج\u2026',
    model_search_no_results: 'لم يتم العثور على نماذج',
    model_scope_advisory: 'ينطبق على هذه المحادثة بدءاً من رسالتك التالية.',
    model_scope_toast: 'ينطبق على هذه المحادثة بدءاً من رسالتك التالية.',
    // commands.js
    cmd_clear: 'مسح رسائل المحادثة',
    cmd_compress: 'ضغط سياق المحادثة يدوياً (الاستخدام: /compress [موضوع التركيز])',
    ctx_compress_hint: 'ضغط السياق لتحرير مساحة ←',
    ctx_compress_action: '⚠ ضغط الآن لتحرير السياق',
    cmd_compact_alias: 'اسم بديل قديم لـ /compress',
    cmd_model: 'تبديل النموذج (مثال /model gpt-4o)',
    cmd_workspace: 'تبديل مساحة العمل بالاسم',
    cmd_terminal: 'فتح طرفية مساحة العمل',
    cmd_new: 'بدء جلسة محادثة جديدة',
    cmd_usage: 'إظهار/إخفاء استخدام الرموز',
    cmd_theme: 'تبديل المظهر (السمة: system/dark/light، الشكل: default/ares/mono/slate/poseidon/sisyphus/charizard)',
    cmd_personality: 'تبديل شخصية الوكيل',
    cmd_skills: 'عرض مهارات Yusuf Mussa المتاحة',
    available_commands: 'الأوامر المتاحة:',
    type_slash: 'اكتب / لعرض الأوامر',
    conversation_cleared: 'تم مسح المحادثة',
    command_label: 'أمر',
    context_compaction_label: 'ضغط السياق',
    preserved_task_list_label: 'قائمة المهام المحفوظة',
    reference_only_label: 'للرجوع فقط',
    model_usage: 'الاستخدام: /model <اسم>',
    no_model_match: 'لا يوجد نموذج مطابق لـ "',
    switched_to: 'تم التبديل إلى ',
    workspace_usage: 'الاستخدام: /workspace <اسم>',
    no_workspace_match: 'لا توجد مساحة عمل مطابقة لـ "',
    switched_workspace: 'تم التبديل إلى مساحة العمل: ',
    workspace_switch_failed: 'فشل تبديل مساحة العمل: ',
    new_session: 'تم إنشاء جلسة جديدة',
    compressing: 'جاري طلب ضغط السياق...',
    compress_running_label: 'جاري الضغط',
    compress_complete_label: 'اكتمل الضغط',
    auto_compress_label: 'ضغط تلقائي',
    compress_failed_label: 'فشل الضغط',
    focus_label: 'التركيز',
    token_usage_on: 'استخدام الرموز مُفعّل',
    token_usage_off: 'استخدام الرموز مُعطّل',
    theme_usage: 'الاستخدام: /theme ',
    theme_set: 'السمة: ',
    no_active_session: 'لا توجد جلسة نشطة',
    cmd_queue: 'وضع رسالة في الانتظار للدور التالي',
    cmd_interrupt: 'إلغاء الدور الحالي وإرسال رسالة جديدة',
    cmd_steer: 'إدخال تعديل أثناء الدور دون مقاطعة الوكيل',
    cmd_queue_no_msg: 'الاستخدام: /queue <رسالة>',
    cmd_queue_not_busy: 'لا توجد مهمة نشطة — أرسل بشكل طبيعي',
    cmd_queue_confirm: 'تم وضع الرسالة في الانتظار',
    cmd_interrupt_no_msg: 'الاستخدام: /interrupt <رسالة>',
    cmd_interrupt_confirm: 'تمت المقاطعة — جاري إرسال رسالة جديدة',
    cmd_steer_no_msg: 'الاستخدام: /steer <رسالة>',
    cmd_steer_fallback: 'التوجيه غير متوفر — تم وضعه في الانتظار للدور التالي',
    cmd_steer_delivered: 'تم تسليم التوجيه — سيراه الوكيل في نتيجة الأداة التالية',
    steer_leftover_queued: 'تم وضع التوجيه في الانتظار للدور التالي',
    busy_steer_fallback: 'التوجيه غير متوفر — تم وضعه في الانتظار للدور التالي',
    busy_interrupt_confirm: 'تمت المقاطعة — جاري إرسال رسالة جديدة',
    settings_label_busy_input_mode: 'وضع الإدخال أثناء الانشغال',
    settings_desc_busy_input_mode: 'يتحكم بما يحدث عند إرسال رسالة أثناء تشغيل الوكيل. الانتظار يضعها في الطابور؛ المقاطعة تلغي وتبدأ من جديد؛ التوجيه يُدخل تعديلاً أثناء الدور دون مقاطعة (يرجع للانتظار عند عدم توفر الوكيل أو البث).',
    settings_busy_input_mode_queue: 'انتظار الرسالة التالية',
    settings_busy_input_mode_interrupt: 'مقاطعة الدور الحالي',
    settings_busy_input_mode_steer: 'توجيه (تعديل أثناء الدور)',
    slash_skill_badge: 'مهارة',
    slash_skill_desc: 'استدعاء هذه المهارة',
    cmd_stop: 'إيقاف الرد الحالي',
    cmd_title: 'عرض أو تعيين عنوان الجلسة',
    cmd_retry: 'إعادة إرسال الرسالة الأخيرة',
    cmd_undo: 'إزالة التبادل الأخير',
    cmd_btw: 'طرح سؤال جانبي (مؤقت)',
    cmd_btw_usage: '/btw <سؤال> — اطرح سؤالاً جانبياً باستخدام سياق الجلسة',
    cmd_background: 'تشغيل موجه في الخلفية',
    cmd_background_usage: '/background <موجه> — تشغيل بالتوازي دون حظر',
    btw_asking: 'جاري طرح السؤال الجانبي...',
    btw_label: 'سؤال جانبي — غير موجود في السجل',
    btw_done: 'تمت الإجابة على السؤال الجانبي',
    btw_no_answer: 'لم يتم استلام إجابة.',
    btw_failed: 'فشل السؤال الجانبي: ',
    bg_running: 'جاري التشغيل في الخلفية...',
    bg_complete: 'اكتملت مهمة الخلفية',
    bg_label: 'نتيجة الخلفية:',
    bg_no_answer: '(لا إجابة)',
    bg_failed: 'فشلت مهمة الخلفية: ',
    undo_exchange: 'تراجع عن التبادل الأخير',
    cmd_status: 'عرض معلومات الجلسة',
    cmd_voice: 'تفعيل/تعطيل إدخال الميكروفون',
    stream_stopped: 'تم إيقاف الرد.',
    no_active_task: 'لا توجد مهمة نشطة لإيقافها.',
    cancel_unavailable: 'الإلغاء غير متوفر.',
    retry_failed: 'فشل إعادة المحاولة: ',
    undo_failed: 'فشل التراجع: ',
    undid_n_messages: 'تمت الإزالة',
    undid_messages_suffix: 'رسالة/رسائل.',
    status_heading: 'حالة الجلسة',
    status_session_id: 'معرّف الجلسة',
    status_title: 'العنوان',
    status_model: 'النموذج',
    status_workspace: 'مساحة العمل',
    status_personality: 'الشخصية',
    status_messages: 'الرسائل',
    status_agent_running: 'الوكيل يعمل',
    status_profile: 'الملف الشخصي',
    status_started: 'بدأت',
    status_tokens: 'الرموز',
    status_no_tokens: 'لم تُستخدم رموز',
    status_unknown: 'غير معروف',
    status_yes: 'نعم',
    status_no: 'لا',
    status_load_failed: 'فشل تحميل الحالة: ',
    title_current: 'العنوان الحالي',
    title_change_hint: 'استخدم `/title <اسم جديد>` لإعادة التسمية.',
    title_set: 'تم تعيين العنوان إلى',
    cmd_webui_only_session: 'هذا الأمر غير متاح للجلسات المستوردة من CLI.',
    cmd_voice_use_mic: 'انقر على زر الميكروفون في المحرر.',
    usage_heading: 'استخدام الرموز',
    usage_default_model: 'الافتراضي',
    usage_unknown: 'غير معروف',
    usage_input_tokens: 'رموز الإدخال',
    usage_output_tokens: 'رموز الإخراج',
    usage_total: 'إجمالي الرموز',
    usage_estimated_cost: 'التكلفة المقدرة',
    usage_settings_tip: 'ملاحظة: تقديرات التكلفة تقريبية.',
    usage_load_failed: 'فشل تحميل الاستخدام: ',
    usage_personality_none: 'بدون',
    no_personalities: 'لم يتم العثور على شخصيات (أضفها إلى ~/.yusuf-mussa/personalities/)',
    available_personalities: 'الشخصيات المتاحة:',
    personality_switch_hint: '\n\nاستخدم `/personality <اسم>` للتبديل، أو `/personality none` للمسح.',
    personalities_load_failed: 'فشل تحميل الشخصيات',
    personality_cleared: 'تم مسح الشخصية',
    personality_set: 'الشخصية: ',
    failed_colon: 'فشل: ',
    // ui.js
    no_workspace: 'لا توجد مساحة عمل',
    terminal_open_title: 'فتح طرفية مساحة العمل',
    terminal_no_workspace_title: 'اختر مساحة عمل لفتح الطرفية',
    terminal_title: 'الطرفية',
    terminal_clear: 'مسح',
    terminal_copy_output: 'نسخ المخرجات',
    terminal_restart: 'إعادة التشغيل',
    terminal_collapse: 'طي',
    terminal_expand: 'توسيع',
    terminal_close: 'إغلاق',
    terminal_input_placeholder: 'تشغيل أمر...',
    terminal_start_failed: 'فشل بدء الطرفية: ',
    terminal_input_failed: 'فشل إدخال الطرفية: ',
    terminal_copy_failed: 'فشل النسخ: ',
    terminal_error: 'خطأ في الطرفية',
    workspace_empty_no_path: 'لم يتم اختيار مساحة عمل. عيّن مساحة عمل في الإعدادات \u2192 مساحة العمل لتصفح الملفات.',
    workspace_empty_dir: 'مساحة العمل هذه فارغة.',
    dialog_confirm_title: 'تأكيد الإجراء',
    dialog_prompt_title: 'أدخل قيمة',
    dialog_confirm_btn: 'تأكيد',
    // workspace.js
    unsaved_confirm: 'لديك تغييرات غير محفوظة في المعاينة. تجاهل والانتقال؟',
    discard: 'تجاهل',
    save: 'حفظ',
    edit: 'تعديل',
    clear: 'مسح',
    create: 'إنشاء',
    remove: 'إزالة',
    save_title: 'حفظ التغييرات',
    edit_title: 'تعديل هذا الملف',
    saved: 'تم الحفظ',
    save_failed: 'فشل الحفظ: ',
    image_load_failed: 'تعذر تحميل الصورة',
    file_open_failed: 'تعذر فتح الملف',
    downloading: (name) => `جاري تنزيل ${name}\u2026`,
    double_click_rename: 'انقر مرتين لإعادة التسمية',
    renamed_to: 'تمت إعادة التسمية إلى ',
    rename_failed: 'فشل إعادة التسمية: ',
    delete_title: 'حذف',
    delete_confirm: (name) => `حذف ${name}؟`,
    delete_dir_confirm: (name) => `حذف المجلد "${name}" وجميع محتوياته؟`,
    rename_title: 'إعادة تسمية',
    rename_prompt: 'الاسم الجديد:',
    deleted: 'تم الحذف ',
    delete_failed: 'فشل الحذف: ',
    new_file_prompt: 'اسم الملف الجديد (مثال notes.md):',
    project_name_prompt: 'اسم المشروع:',
    created: 'تم الإنشاء ',
    create_failed: 'فشل الإنشاء: ',
    new_folder_prompt: 'اسم المجلد الجديد:',
    folder_created: 'تم إنشاء المجلد ',
    folder_create_failed: 'فشل إنشاء المجلد: ',
    workspace_auto_create_folder: 'إنشاء المجلد إذا لم يكن موجوداً',
    folder_add_as_space_btn: 'إضافة كمساحة',
    folder_add_as_space_msg: 'إضافة هذا المجلد كمساحة جديدة في قائمة مساحات العمل؟',
    folder_add_as_space_title: 'إضافة كمساحة؟',
    remove_title: 'إزالة',
    empty_dir: '(فارغ)',
    upload_failed: 'فشل الرفع: ',
    all_uploads_failed: (n) => `فشل جميع ${n} عملية رفع`,
    archive_extracted: (n, c) => `تم استخراج ${n} ملف(ات) من ${c} أرشيف(ات)`,
    session_pin: 'تثبيت المحادثة',
    session_unpin: 'إلغاء تثبيت المحادثة',
    session_pin_desc: 'إبقاء هذه المحادثة في الأعلى',
    session_unpin_desc: 'إزالة من المثبتة',
    session_pin_failed: 'فشل التثبيت: ',
    session_move_project: 'نقل إلى مشروع',
    session_move_project_desc_has: 'تغيير المشروع لهذه المحادثة',
    session_move_project_desc_none: 'تعيين مشروع لهذه المحادثة',
    session_archive: 'أرشفة المحادثة',
    session_restore: 'استعادة المحادثة',
    session_archive_desc: 'إخفاء هذه المحادثة حتى تظهر المؤرشفة',
    session_restore_desc: 'إعادة هذه المحادثة إلى القائمة الرئيسية',
    session_archived: 'تم أرشفة الجلسة',
    session_restored: 'تم استعادة الجلسة',
    session_archive_failed: 'فشل الأرشفة: ',
    session_duplicate: 'تكرار المحادثة',
    session_duplicate_desc: 'إنشاء نسخة بنفس مساحة العمل والنموذج',
    session_duplicated: 'تم تكرار الجلسة',
    session_duplicate_failed: 'فشل التكرار: ',
    session_delete: 'حذف المحادثة',
    session_delete_desc: 'إزالة هذه المحادثة نهائياً',
    session_select_mode: 'تحديد',
    session_select_mode_desc: 'تحديد محادثات لإدارتها دفعة واحدة',
    session_select_all: 'تحديد الكل',
    session_deselect_all: 'إلغاء تحديد الكل',
    session_selected_count: '{0} محددة',
    session_batch_archive: 'أرشفة',
    session_batch_delete: 'حذف',
    session_batch_move: 'نقل إلى مشروع',
    session_batch_delete_confirm: 'حذف {0} محادثات؟',
    session_batch_archive_confirm: 'أرشفة {0} محادثات؟',
    session_no_selection: 'لم يتم تحديد محادثات',
    // settings panel
    settings_heading_title: 'مركز التحكم',
    settings_heading_subtitle: 'التفضيلات، أدوات المحادثة، وعناصر التحكم بالنظام.',
    settings_section_conversation_title: 'المحادثة',
    settings_section_appearance_title: 'المظهر',
    settings_section_appearance_meta: 'السمة، ألوان التمييز، والنمط البصري.',
    settings_section_preferences_title: 'التفضيلات',
    settings_section_preferences_meta: 'الإعدادات الافتراضية وسلوك واجهة Yusuf Mussa Web UI.',
    settings_section_system_title: 'النظام',
    settings_section_system_meta: 'إصدار المثيل وأدوات التحكم بالوصول.',
    settings_check_now: 'تحقق الآن',
    settings_checking: 'جاري التحقق\u2026',
    settings_up_to_date: 'مُحدَّث \u2713',
    settings_updates_available: '{count} تحديث(ات) متاحة',
    settings_updates_disabled: 'فحوصات التحديث معطلة',
    settings_update_check_failed: 'فشل فحص التحديث',
    settings_label_workspace_panel_open: 'إبقاء لوحة مساحة العمل مفتوحة افتراضياً',
    settings_desc_workspace_panel_open: 'عند التفعيل، تفتح لوحة مساحة العمل / متصفح الملفات تلقائياً مع كل جلسة جديدة. لا يزال بإمكانك إغلاقها يدوياً في أي وقت.',
    open_in_browser: 'فتح في المتصفح',
    settings_dropdown_conversation: 'المحادثة',
    settings_dropdown_appearance: 'المظهر',
    settings_dropdown_preferences: 'التفضيلات',
    settings_dropdown_providers: 'المزودون',
    settings_dropdown_system: 'النظام',
    settings_tab_conversation: 'المحادثة',
    settings_tab_appearance: 'المظهر',
    settings_tab_preferences: 'التفضيلات',
    settings_tab_system: 'النظام',
    settings_title: 'الإعدادات',
    settings_save_btn: 'حفظ الإعدادات',
    settings_label_model: 'النموذج الافتراضي',
    settings_desc_model: 'يُستخدم للمحادثات الجديدة. المحادثات الحالية تحافظ على نموذجها المحدد.',
    settings_label_send_key: 'مفتاح الإرسال',
    settings_label_theme: 'السمة',
    settings_label_skin: 'الشكل',
    settings_label_font_size: 'حجم الخط',
    font_size_small: 'صغير',
    font_size_default: 'افتراضي',
    font_size_large: 'كبير',
    settings_autosave_saving: 'جاري الحفظ\u2026',
    settings_autosave_saved: 'تم الحفظ',
    settings_autosave_failed: 'فشل الحفظ',
    settings_autosave_retry: 'إعادة المحاولة',
    settings_label_language: 'اللغة',
    settings_label_token_usage: 'إظهار استخدام الرموز',
    settings_label_sidebar_density: 'كثافة الشريط الجانبي',
    cmd_reasoning: 'إظهار/إخفاء التفكير، تعيين مستوى الجهد، أو التحقق من الحالة الحالية',
    settings_label_cli_sessions: 'إظهار جلسات الوكيل',
    settings_label_sync_insights: 'مزامنة مع التحليلات',
    settings_label_check_updates: 'التحقق من التحديثات',
    settings_label_bot_name: 'اسم المساعد',
    settings_label_password: 'كلمة مرور الوصول',
    settings_saved: 'تم حفظ الإعدادات',
    settings_save_failed: 'فشل الحفظ: ',
    settings_load_failed: 'فشل تحميل الإعدادات: ',
    settings_saved_pw: 'تم حفظ الإعدادات — تم تفعيل حماية كلمة المرور وهذا المتصفح يبقى مسجلاً الدخول',
    settings_saved_pw_updated: 'تم حفظ الإعدادات — تم تحديث كلمة المرور',
    // login page
    login_title: 'تسجيل الدخول',
    login_subtitle: 'أدخل كلمة المرور للمتابعة',
    login_placeholder: 'كلمة المرور',
    login_btn: 'تسجيل الدخول',
    login_invalid_pw: 'كلمة مرور غير صالحة',
    login_conn_failed: 'فشل الاتصال',
    // Sidebar & Tabs
    tab_chat: 'محادثة',
    tab_tasks: 'المهام',
    tab_skills: 'المهارات',
    ...
    session_time_unknown: 'غير معروف',
    session_time_minutes_ago: (n) => `${n}د`,
    session_time_hours_ago: (n) => `${n}س`,
    session_time_days_ago: (n) => `${n}ي`,
    session_time_last_week: '1أ',
    session_time_bucket_today: 'اليوم',
    session_time_bucket_yesterday: 'أمس',
    session_time_bucket_this_week: 'هذا الأسبوع',
    session_time_bucket_last_week: 'الأسبوع الماضي',
    session_time_bucket_older: 'أقدم',
    scheduled_jobs: 'المهام المجدولة',
    new_job: 'مهمة جديدة',
    loading: 'جاري التحميل...',
    search_skills: 'البحث في المهارات...',
    new_skill: 'مهارة جديدة',
    personal_memory: 'الذاكرة الشخصية',
    current_task_list: 'قائمة المهام الحالية',
    workspace_desc: 'إضافة وتبديل مساحات العمل لجلساتك.',
    session_meta_messages: (n) => `${n} رسالة`,
    new_profile: 'ملف شخصي جديد',
    transcript: 'النسخة',
    download_transcript: 'تنزيل كـ Markdown',
    import: 'استيراد',
    // Settings detail
    settings_label_sound: 'صوت الإشعار',
    settings_desc_sound: 'تشغيل صوت عند انتهاء المساعد من الرد.',
    // TTS (#499)
    tts_listen: 'استمع',
    tts_not_supported: 'التركيب الصوتي غير مدعوم في هذا المتصفح.',
    settings_label_tts: 'تحويل النص إلى كلام للردود',
    settings_desc_tts: 'إظهار زر مكبر صوت على كل رسالة مساعد لقراءتها بصوت عالٍ باستخدام التركيب الصوتي للمتصفح.',
    settings_label_tts_auto_read: 'القراءة التلقائية للردود بصوت عالٍ',
    settings_desc_tts_auto_read: 'القراءة التلقائية لكل رد جديد من المساعد عند انتهائه. يتوقف عند بدء الكتابة.',
    settings_label_tts_voice: 'الصوت',
    settings_desc_tts_voice: 'الصوت المفضل. يتم تعبئته من الأصوات المتاحة في المتصفح.',
    settings_label_tts_rate: 'سرعة الكلام',
    settings_label_tts_pitch: 'نبرة الصوت',
    settings_label_notifications: 'إشعارات المتصفح',
    settings_desc_notifications: 'إظهار إشعار نظام عند اكتمال الرد أثناء عمل التطبيق في الخلفية.',
    settings_desc_token_usage: 'يعرض عدد رموز الإدخال/الإخراج أسفل كل رد من المساعد. يُفعّل أيضاً بـ /usage.',
    settings_sidebar_density_compact: 'مضغوط',
    settings_sidebar_density_detailed: 'مفصّل',
    settings_desc_sidebar_density: 'يتحكم في مقدار البيانات الوصفية التي تعرضها قائمة الجلسات في الشريط الجانبي الأيسر.',
    settings_label_auto_title_refresh: 'تحديث العنوان التكيفي',
    settings_auto_title_refresh_off: 'إيقاف',
    settings_auto_title_refresh_5: 'كل 5 تبادلات',
    settings_auto_title_refresh_10: 'كل 10 تبادلات',
    settings_auto_title_refresh_20: 'كل 20 تبادلاً',
    settings_desc_auto_title_refresh: 'إعادة إنشاء عنوان الجلسة تلقائياً بناءً على آخر تبادل، للحفاظ على ملاءمته مع تطور المحادثة. يتطلب إعداد نموذج توليد عناوين LLM.',
    settings_desc_cli_sessions: 'دمج الجلسات من Yusuf Mussa CLI (state.db) في قائمة الجلسات. انقر على جلسة CLI لاستيرادها ومتابعة المحادثة.',
    settings_desc_sync_insights: 'يعكس استخدام الرموز من WebUI إلى state.db حتى يتضمن ym /insights بيانات جلسات المتصفح. معطّل افتراضياً.',
    settings_desc_check_updates: 'إظهار شعار عند توفر إصدارات أحدث من WebUI أو الوكيل. يشغّل git fetch في الخلفية بشكل دوري.',
    settings_desc_bot_name: 'الاسم المعروض للمساعد في جميع أنحاء الواجهة. الافتراضي هو Yusuf Mussa.',
    settings_desc_password: 'أدخل كلمة مرور جديدة لتعيينها أو تغييرها. اتركها فارغة للإبقاء على الإعداد الحالي.',
    password_placeholder: 'أدخل كلمة المرور الجديدة\u2026',
    disable_auth: 'تعطيل المصادقة',
    sign_out: 'تسجيل الخروج',
    // Providers panel
    providers_tab_title: 'المزودون',
    providers_section_title: 'المزودون',
    providers_section_meta: 'إدارة مفاتيح API لمزودي الذكاء الاصطناعي. التغييرات تسري فوراً.',
    providers_status_configured: 'مفتاح API مُعد',
    providers_status_not_configured: 'لا يوجد مفتاح API',
    providers_status_oauth: 'OAuth',
    providers_status_api_key: 'مفتاح API',
    providers_status_not_configured_label: 'غير مُعد',
    providers_oauth_hint: 'تمت المصادقة عبر OAuth. لا حاجة لمفتاح API.',
    providers_oauth_config_yaml_hint: 'تم إعداد الرمز عبر config.yaml. للتحديث، عدّل قسم المزودين في config.yaml أو شغّل ym auth.',
    providers_oauth_not_configured_hint: 'لم تتم المصادقة. شغّل ym auth في الطرفية لإعداد هذا المزود.',
    providers_save: 'حفظ',
    providers_remove: 'إزالة',
    providers_saving: 'جاري الحفظ\u2026',
    providers_removing: 'جاري الإزالة\u2026',
    providers_enter_key: 'يرجى إدخال مفتاح API',
    providers_empty: 'لم يتم العثور على مزودين قابلين للإعداد.',
    providers_key_updated: 'تم حفظ مفتاح API',
    providers_key_removed: 'تم إزالة مفتاح API',
    providers_key_placeholder_new: 'sk-...',
    providers_key_placeholder_replace: 'أدخل مفتاحاً جديداً للاستبدال\u2026',
    cancel: 'إلغاء',
    create_job: 'إنشاء مهمة',
    save_skill: 'حفظ المهارة',
    editing: 'جاري التعديل',
    // Empty state
    empty_title: 'بم يمكنني المساعدة؟',
    empty_subtitle: 'اسأل أي شيء، شغّل أوامر، استكشف الملفات، أو أدر مهامك المجدولة.',
    suggest_files: 'ما الملفات الموجودة في مساحة العمل هذه؟',
    suggest_schedule: 'ما المقرر في جدولي اليوم؟',
    suggest_plan: 'ساعدني في تخطيط مشروع صغير.',
    suggest_search: 'ابحث في الويب عن آخر الأخبار',
    suggest_code: 'اكتب وشغّل سكربت بايثون',
    suggest_arabic: 'ساعدني بالعربية',
    // onboarding
    onboarding_badge: 'التشغيل الأول',
    onboarding_title: 'مرحباً بك في Yusuf Mussa Web UI',
    onboarding_lead: 'سيقوم إعداد موجز بالتحقق من Yusuf Mussa، وحفظ إعداد مزود حقيقي، واختيار مساحة عمل ونموذج، وحماية التطبيق اختيارياً بكلمة مرور.',
    onboarding_back: 'رجوع',
    onboarding_continue: 'متابعة',
    onboarding_skip: 'تخطي الإعداد',
    onboarding_skipped: 'تم تخطي الإعداد — استخدام الإعداد الحالي.',
    onboarding_open: 'فتح Yusuf Mussa',
    onboarding_step_system_title: 'فحص النظام',
    onboarding_step_system_desc: 'التحقق من Yusuf Mussa Agent ورؤية الإعدادات.',
    onboarding_step_setup_title: 'إعداد المزود',
    onboarding_step_setup_desc: 'حفظ الحد الأدنى من إعداد مزود Yusuf Mussa.',
    onboarding_step_workspace_title: 'مساحة العمل + النموذج',
    onboarding_step_workspace_desc: 'اختيار الإعدادات الافتراضية للجلسات الجديدة والمحادثة.',
    onboarding_step_password_title: 'كلمة مرور اختيارية',
    onboarding_step_password_desc: 'حماية Web UI قبل مشاركته.',
    onboarding_step_finish_title: 'إنهاء',
    onboarding_step_finish_desc: 'مراجعة والدخول إلى التطبيق.',
    onboarding_notice_system_ready: 'يبدو أن Yusuf Mussa Agent يمكن الوصول إليه من Web UI.',
    onboarding_notice_system_unavailable: 'Yusuf Mussa Agent غير متاح بالكامل بعد. يمكن لبرنامج التثبيت تثبيته، لكن إعداد المزود قد لا يزال يتطلب طرفية.',
    onboarding_check_agent: 'Yusuf Mussa Agent',
    onboarding_check_agent_ready: 'تم اكتشافه وقابل للاستيراد',
    onboarding_check_agent_missing: 'مفقود أو قابل للاستيراد جزئياً',
    onboarding_check_password: 'كلمة المرور',
    onboarding_check_password_enabled: 'مُفعّلة بالفعل',
    onboarding_check_password_disabled: 'غير مُفعّلة بعد',
    onboarding_check_provider: 'إعداد المزود',
    onboarding_check_provider_ready: 'جاهز للمحادثة',
    onboarding_check_provider_partial: 'محفوظ لكن غير مكتمل',
    onboarding_check_provider_pending: 'يحتاج إلى تحقق',
    onboarding_config_file: 'ملف الإعداد:',
    onboarding_env_file: 'ملف .env:',
    onboarding_unknown: 'غير معروف',
    onboarding_current_provider: 'الإعداد الحالي:',
    onboarding_missing_imports: 'عمليات الاستيراد المفقودة:',
    onboarding_notice_setup_required: 'اختر مسار مزود بسيط هنا. تدفقات OAuth المتقدمة لا تزال تخص Yusuf Mussa CLI حالياً.',
    onboarding_notice_setup_already_ready: 'تم اكتشاف إعداد مزود Yusuf Mussa يعمل بالفعل. يمكنك الإبقاء عليه أو استبداله هنا.',
    onboarding_oauth_provider_ready_title: 'المزود مصادق عليه بالفعل',
    onboarding_oauth_provider_ready_body: 'هذه المثيل مُعد لاستخدام مزود OAuth (<strong>{provider}</strong>) تم إعداده عبر Yusuf Mussa CLI. لا حاجة لمفتاح API هنا — انقر متابعة لإنهاء الإعداد.',
    onboarding_oauth_provider_not_ready_title: 'مزود OAuth غير مصادق عليه بعد',
    onboarding_oauth_provider_not_ready_body: 'هذه المثيل مُعد لاستخدام <strong>{provider}</strong>، والذي يستخدم OAuth بدلاً من مفتاح API. شغّل <code>ym auth</code> أو <code>ym model</code> في طرفية للمصادقة، ثم أعد تحميل Web UI.',
    onboarding_oauth_switch_hint: 'أو اختر مزوداً مختلفاً أدناه للتبديل إلى إعداد مفتاح API:',
    onboarding_notice_workspace: 'هذه القيم تعيد استخدام نفس واجهات برمجة الإعدادات كالتطبيق العادي.',
    onboarding_workspace_label: 'مساحة العمل',
    onboarding_workspace_or_path: 'أو أدخل مسار مساحة العمل',
    onboarding_workspace_placeholder: '/home/you/workspace',
    onboarding_provider_label: 'وضع الإعداد',
    onboarding_quick_setup_badge: 'إعداد سريع',
    provider_category_easy_start: 'بدء سهل',
    provider_category_self_hosted: 'مفتوح / مستضاف ذاتياً',
    provider_category_specialized: 'متخصص',
    onboarding_api_key_label: 'مفتاح API',
    onboarding_api_key_placeholder: 'اتركه فارغاً للإبقاء على المفتاح المحفوظ',
    onboarding_api_key_help_prefix: 'يُحفظ كسر في ملف Yusuf Mussa .env باستخدام',
    onboarding_base_url_label: 'الرابط الأساسي',
    onboarding_base_url_placeholder: 'https://your-endpoint.example/v1',
    onboarding_base_url_help: 'استخدم هذا لمُوجِّهات OpenAI المتوافقة، الخوادم المستضافة ذاتياً، LiteLLM، Ollama، LM Studio، vLLM، أو نقاط نهاية مشابهة.',
    onboarding_model_label: 'النموذج الافتراضي',
    onboarding_workspace_help: 'اختر النموذج الذي يجب أن يستخدمه Yusuf Mussa للمحادثات الجديدة بعد اكتمال الإعداد.',
    onboarding_custom_model_placeholder: 'your-model-name',
    onboarding_custom_model_help: 'لنقاط النهاية المخصصة، أدخل معرّف النموذج الدقيق الذي يتوقعه خادمك.',
    onboarding_notice_password_enabled: 'كلمة المرور مُعدَّة بالفعل. أدخل واحدة جديدة فقط إذا كنت تريد استبدالها.',
    onboarding_notice_password_recommended: 'اختياري لكن موصى به إذا كنت ستعرض الواجهة خارج localhost.',
    onboarding_password_label: 'كلمة المرور (اختياري)',
    onboarding_password_placeholder: 'اتركها فارغة للتخطي',
    onboarding_password_help: 'تُخزَّن كلمات المرور عبر واجهة برمجة الإعدادات الحالية ويتم تجزئتها من جانب الخادم.',
    onboarding_notice_finish: 'يمكنك إعادة فتح الإعدادات لاحقاً لتغيير أي من هذا.',
    onboarding_not_set: 'غير معين',
    onboarding_password_will_enable: 'سيتم تفعيلها',
    onboarding_password_will_replace: 'سيتم استبدالها',
    onboarding_password_keep_existing: 'الإبقاء على كلمة المرور الحالية',
    onboarding_password_remains_disabled: 'ستبقى معطلة',
    onboarding_password_skipped: 'تم التخطي حالياً',
    onboarding_finish_help: 'الإنهاء يخزّن <code>onboarding_completed</code> في الإعدادات وينقلك إلى التطبيق العادي.',
    onboarding_error_choose_workspace: 'اختر مساحة عمل قبل المتابعة.',
    onboarding_error_choose_model: 'اختر نموذجاً قبل المتابعة.',
    onboarding_error_provider_required: 'اختر وضع إعداد قبل المتابعة.',
    onboarding_error_base_url_required: 'الرابط الأساسي مطلوب لنقاط النهاية المخصصة.',
    onboarding_error_workspace_required: 'مساحة العمل مطلوبة.',
    onboarding_error_model_required: 'النموذج مطلوب.',
    ...
    dismiss: 'تجاهل',
    disable: 'تعطيل',
    cron_no_jobs: 'لم يتم العثور على مهام مجدولة.',
    cron_status_off: 'مطفأ',
    cron_status_paused: 'متوقف مؤقتاً',
    cron_status_error: 'خطأ',
    cron_status_active: 'نشط',
    cron_status_running: 'جاري التشغيل\u2026',
    cron_status_needs_attention: 'يحتاج انتباهاً',
    cron_attention_desc: 'هذه المهمة المتكررة ليس لها وقت تشغيل تالي. قد يكون المجدول قد فشل في حساب وقت التشغيل التالي.',
    cron_attention_croniter_hint: 'قد يفتقد وقت تشغيل Gateway لحزمة croniter. أعد تشغيل Gateway مع دعم cron، ثم استأنف هذه المهمة.',
    cron_attention_resume: 'استئناف وإعادة الحساب',
    cron_attention_run_once: 'تشغيل مرة واحدة الآن',
    cron_attention_copy_diagnostics: 'نسخ التشخيصات',
    cron_diagnostics_copied: 'تم نسخ تشخيصات Cron',
    cron_next: 'التالي',
    cron_last: 'الأخير',
    cron_run_now: 'تشغيل الآن',
    cron_pause: 'إيقاف مؤقت',
    cron_resume: 'استئناف',
    cron_job_name_placeholder: 'اسم المهمة',
    cron_schedule_placeholder: 'الجدول',
    cron_prompt_placeholder: 'الموجه',
    cron_last_output: 'آخر مخرجات',
    cron_all_runs: 'جميع التشغيلات',
    cron_hide_runs: 'إخفاء التشغيلات',
    cron_no_runs_yet: '(لم يتم التشغيل بعد)',
    cron_schedule_required_example: 'الجدول مطلوب (مثال "0 9 * * *" أو "every 1h")',
    cron_schedule_required: 'الجدول مطلوب',
    cron_prompt_required: 'الموجه مطلوب',
    cron_job_created: 'تم إنشاء المهمة',
    cron_duplicate: 'تكرار',
    cron_duplicated: 'تم تكرار المهمة (متوقف مؤقتاً)',
    cron_job_triggered: 'تم تشغيل المهمة',
    cron_job_paused: 'تم إيقاف المهمة مؤقتاً',
    cron_job_resumed: 'تم استئناف المهمة',
    cron_job_updated: 'تم تحديث المهمة',
    cron_delete_confirm_title: 'حذف مهمة cron',
    cron_delete_confirm_message: 'لا يمكن التراجع عن هذا الإجراء.',
    cron_job_deleted: 'تم حذف المهمة',
    cron_completion_status: (name, status) => `Cron "${name}" ${status}`,
    status_failed: 'فشل',
    status_completed: 'اكتمل',
    todos_no_active: 'لا توجد قائمة مهام نشطة في هذه الجلسة.',
    clear_conversation_title: 'مسح المحادثة',
    clear_conversation_message: 'مسح جميع الرسائل؟ لا يمكن التراجع عن هذا الإجراء.',
    clear_failed: 'فشل المسح: ',
    skills_no_match: 'لا توجد مهارات مطابقة.',
    linked_files: 'الملفات المرتبطة',
    skill_load_failed: 'تعذر تحميل المهارة: ',
    skill_file_load_failed: 'تعذر تحميل الملف: ',
    ...
    skill_delete_confirm: 'حذف المهارة "{0}"؟',
    skills_empty_title: 'اختر مهارة',
    skills_empty_sub: 'اختر مهارة من الشريط الجانبي لعرض محتوياتها، أو أنشئ واحدة جديدة.',
    skills_edit: 'تعديل',
    skills_delete: 'حذف',
    skills_back_to: 'العودة إلى {0}',
    tasks_empty_title: 'اختر مهمة مجدولة',
    tasks_empty_sub: 'اختر مهمة من الشريط الجانبي لعرض تفاصيلها وتشغيلاتها، أو أنشئ واحدة جديدة.',
    workspaces_empty_title: 'اختر مساحة',
    workspaces_empty_sub: 'اختر مساحة من الشريط الجانبي لعرض ملفاتها وإعداداتها، أو أضف واحدة جديدة.',
    profiles_empty_title: 'اختر ملفاً شخصياً',
    profiles_empty_sub: 'اختر ملفاً شخصياً لوكيل من الشريط الجانبي لعرض إعداداته وتعديلها، أو أنشئ واحداً جديداً.',
    memory_notes_label: 'الذاكرة (ملاحظات)',
    memory_saved: 'تم حفظ الذاكرة',
    my_notes: 'ملاحظاتي',
    user_profile: 'الملف الشخصي',
    no_notes_yet: 'لا توجد ملاحظات بعد.',
    no_profile_yet: 'لا يوجد ملف شخصي بعد.',
    workspace_choose_path: 'اختر مسار مساحة العمل',
    workspace_choose_path_meta: 'أضف مساراً محققاً وبدّل هذه المحادثة',
    workspace_manage: 'إدارة مساحات العمل',
    workspace_manage_meta: 'فتح لوحة المساحات',
    workspace_use_title: 'استخدام في الجلسة الحالية',
    workspace_use: 'استخدام',
    workspace_add_path_placeholder: 'أضف مسار مساحة العمل (مثال /home/user/my-project)',
    workspace_paths_validated_hint: 'يتم التحقق من المسارات كأدلة موجودة قبل الحفظ.',
    workspace_drag_hint: 'اسحب لإعادة الترتيب',
    workspace_reorder_failed: 'فشل إعادة الترتيب',
    workspace_added: 'تمت إضافة مساحة العمل',
    workspace_renamed: 'تمت إعادة تسمية مساحة العمل',
    workspace_remove_confirm_title: 'إزالة مساحة العمل',
    workspace_remove_confirm_message: (path) => `إزالة "${path}"؟`,
    workspace_removed: 'تمت إزالة مساحة العمل',
    workspace_switch_prompt_title: 'تبديل مساحة العمل',
    workspace_switch_prompt_message: 'أدخل مسار مساحة عمل مطلق لإضافته والتبديل إليه في هذه المحادثة.',
    workspace_switch_prompt_confirm: 'تبديل',
    workspace_switch_prompt_placeholder: '/Users/you/project',
    workspace_not_added: 'لم تتم إضافة مساحة العمل',
    workspace_already_saved: 'مساحة العمل محفوظة بالفعل — اخترها من القائمة',
    workspace_busy_switch: 'لا يمكن تبديل مساحة العمل أثناء تشغيل الوكيل',
    discard_file_edits_title: 'تجاهل تعديلات الملف؟',
    discard_file_edits_message: 'تبديل مساحات العمل سيتجاهل تعديلات الملف غير المحفوظة في المعاينة.',
    workspace_switched_to: (name) => `تم التبديل إلى ${name}`,
    profiles_no_profiles: 'لم يتم العثور على ملفات شخصية.',
    profile_api_keys_configured: 'مفاتيح API مُعدَّة',
    profile_gateway_running: 'Gateway يعمل',
    profile_gateway_stopped: 'Gateway متوقف',
    profile_active: 'نشط',
    profile_no_configuration: 'بدون إعداد',
    profile_skill_count: (count) => `${count} ${count === 1 ? 'مهارة' : 'مهارات'}`,
    profile_use: 'استخدام',
    profile_switch_title: 'التبديل إلى هذا الملف الشخصي',
    profile_delete_title: 'حذف هذا الملف الشخصي',
    profile_default_label: '(افتراضي)',
    profile_name_placeholder: 'اسم الملف الشخصي (أحرف صغيرة، a-z 0-9 شرطات)',
    profile_clone_label: 'استنساخ الإعداد من الملف الشخصي النشط',
    profile_base_url_placeholder: 'الرابط الأساسي (اختياري، مثال http://localhost:11434)',
    profile_api_key_placeholder: 'مفتاح API (اختياري)',
    manage_profiles: 'إدارة الملفات الشخصية',
    profiles_load_failed: 'فشل تحميل الملفات الشخصية',
    profiles_busy_switch: 'لا يمكن تبديل الملفات الشخصية أثناء تشغيل الوكيل',
    profile_switched_new_conversation: (name) => `تم التبديل إلى الملف الشخصي: ${name} — بدأت محادثة جديدة`,
    profile_switched: (name) => `تم التبديل إلى الملف الشخصي: ${name}`,
    profile_name_rule: 'أحرف صغيرة، أرقام، شرطات، شرطات سفلية فقط',
    profile_base_url_rule: 'يجب أن يبدأ الرابط الأساسي بـ http:// أو https://',
    profile_created: (name) => `تم إنشاء الملف الشخصي: ${name}`,
    profile_delete_confirm_title: (name) => `حذف الملف الشخصي "${name}"؟`,
    profile_delete_confirm_message: 'سيتم حذف جميع الجلسات والإعدادات والمهارات والذاكرة لهذا الملف الشخصي نهائياً. لا يمكن التراجع عن هذا الإجراء.',
    profile_deleted: (name) => `تم حذف الملف الشخصي: ${name}`,
    active_conversation_none: 'لم يتم تحديد محادثة نشطة.',
    active_conversation_meta: (title, count) => `${title} · ${count} رسالة`,
    settings_unsaved_changes: 'لديك تغييرات غير محفوظة.',
    sign_out_failed: 'فشل تسجيل الخروج: ',
    disable_auth_confirm_title: 'تعطيل حماية كلمة المرور',
    disable_auth_confirm_message: 'سيتمكن أي شخص من الوصول إلى هذه المثيل.',
    auth_disabled: 'تم تعطيل المصادقة — تمت إزالة حماية كلمة المرور',
    disable_auth_failed: 'فشل تعطيل المصادقة: ',
    bg_error_single: (title) => `"${title}" واجه خطأ`,
    bg_error_multi: (count) => `${count} جلسات واجهت خطأ`,
    // skill form
    skill_name: 'الاسم',
    skill_category: 'الفئة',
    skill_category_placeholder: 'اختياري، مثال devops',
    skill_content: 'محتوى SKILL.md',
    skill_content_placeholder: 'واجهة YAML + محتوى markdown',
    skill_rename_not_supported: 'إعادة تسمية المهارة غير مدعومة. أنشئ مهارة جديدة واحذف القديمة لإعادة التسمية.',
    skill_metadata: 'البيانات الوصفية',
    // cron form
    cron_name_label: 'الاسم',
    cron_name_placeholder: 'اختياري',
    cron_schedule_label: 'الجدول',
    cron_schedule_hint: 'تعبير Cron أو اختصار مثل \'every 1h\'.',
    cron_prompt_label: 'الموجه',
    cron_deliver_label: 'إرسال المخرجات إلى',
    cron_deliver_local: 'محلي (حفظ المخرجات فقط)',
    cron_skills_label: 'المهارات',
    cron_skills_placeholder: 'إضافة مهارات (اختياري)\u2026',
    cron_skills_edit_hint: 'قائمة المهارات غير قابلة للتعديل بعد الإنشاء.',
    // workspace form
    workspace_name_label: 'الاسم',
    workspace_name_placeholder: 'اسم ودود اختياري',
    workspace_path_label: 'المسار',
    workspace_path_required: 'المسار مطلوب',
    workspace_path_readonly: 'لا يمكن تغيير المسار. إعادة تسمية فقط.',
    workspace_new_title: 'مساحة جديدة',
    // profile form
    profile_name_label: 'الاسم',
    profile_base_url_label: 'الرابط الأساسي',
    profile_api_key_label: 'مفتاح API',
    cmd_yolo: 'تبديل وضع YOLO (تخطي الموافقات)',
    yolo_no_session: 'لا توجد جلسة نشطة',
    yolo_enabled: '⚡ وضع YOLO مُفعّل — تم تخطي الموافقات في هذه الجلسة',
    yolo_disabled: 'وضع YOLO معطّل',
    yolo_pill_label: 'YOLO',
    yolo_pill_title_active: 'وضع YOLO نشط — انقر للتعطيل',
    approval_skip_all: '⚡ تخطي الكل في هذه الجلسة',
    approval_skip_all_title: 'تخطي جميع طلبات الموافقة في هذه الجلسة',
    // composer action tooltips
    composer_send: 'إرسال الرسالة',
    composer_queue: 'وضع الرسالة في الانتظار',
    composer_interrupt: 'مقاطعة وإرسال',
    composer_steer: 'توجيه الرد الحالي',
    composer_stop: 'إيقاف التوليد',
    composer_disabled_clarify: 'الرد على طلب التوضيح',
    composer_disabled_compression: 'في انتظار اكتمال الضغط',
    composer_disabled_empty: 'اكتب رسالة للإرسال',
    media_audio_label: 'صوت',
    media_svg_label: 'مخطط',
    media_video_label: 'فيديو',
    csv_loading: 'جاري تحميل CSV',
    csv_too_large: 'ملف CSV كبير جداً للعرض المباشر',
    csv_no_data: 'ملف CSV لا يحتوي على بيانات كافية للعرض كجدول',
    csv_error: 'فشل تحميل ملف CSV',
    csv_header_note: 'الصف الأول يظهر كعنوان الجدول',
    excalidraw_loading: 'جاري تحميل المخطط',
    excalidraw_too_large: 'ملف Excalidraw كبير جداً للعرض المباشر',
    excalidraw_invalid: 'تنسيق ملف Excalidraw غير صالح',
    excalidraw_error: 'فشل تحميل ملف Excalidraw',
    excalidraw_label: 'مخطط',
    excalidraw_download: 'تنزيل',
    excalidraw_empty: 'مخطط فارغ',
    excalidraw_render_error: 'فشل عرض المخطط',
    excalidraw_simplified: 'معاينة SVG مبسطة — ليست مطابقة بكسلياً لكانفاس Excalidraw',
  },
...
function setLocale(lang) {
  ...
  localStorage.setItem('ym-lang', resolved);
 ...
 */
function loadLocale() {
  setLocale(resolvePreferredLocale(null, localStorage.getItem('ym-lang')));
}
