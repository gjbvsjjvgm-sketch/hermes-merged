...
def _personalities_dir():
    ...
    conftest sets YM_HOME=TEST_STATE_DIR in the server's environment.
    The server's api/profiles._DEFAULT_YM_HOME resolves to TEST_STATE_DIR,
    so get_active_ym_home() returns TEST_STATE_DIR, and personalities
...
def test_personalities_returns_empty_when_none_configured():
    ...
    d, status = get("/api/personalities")
    assert status == 200
    # May or may not have personalities depending on the real ~/.yusuf-mussa/config.yaml
