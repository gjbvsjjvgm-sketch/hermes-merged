...
Users with older agent versions (missing 'source' column in state.db) got
...
class TestCliSessionsErrorSurface:
    ...
    def test_missing_source_column_suggests_upgrade(self):
        """Warning message must suggest upgrading agent."""
        assert "Upgrade agent" in combined_src or "upgrade agent" in combined_src.lower()
